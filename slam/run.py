...
def main():
    ...
    Q = np.diag(beta ** 2)
    sam = Sam(initial_state, alphas, slam_type=args.filter_name, data_association='known', update_type='batch', Q=Q)
    ...
    sam.solve_last(method='lm')
    sam.graph_viz(last=True)
    sam.plot_errors(dt=args.dt)
    sam.matrices()
